...
def build_svg(data: dict) -> str:
    ...
    for lang, count in langs[:5]:
        ...
        lang_labels.append(
            ...
            f'<text x="{label_x + 13}" y="{label_y}" fill="#8b949e" font-size="10">'
            f'{lang} {pct}%</text>'
        )
        label_x += 80
        if label_x > W - 80:
    ...
    for i, g in enumerate(recent):
        ...
        recent_rows.append(
            ...
            f'<text x="38" y="{ry + 5}" fill="#e6edf3" font-size="11">{_esc(desc)}</text>'
            f'<text x="{W - 20}" y="{ry + 5}" fill="#8b949e" font-size="10" text-anchor="end">{g["updated_at"]}</text>'
        )
    ...
    # ── Assemble SVG ──
    svg = f"""<svg width="{W}" height="{H_TOTAL}" viewBox="0 0 {W} {H_TOTAL}"
  ...
  <!-- BG -->
  <rect width="{W}" height="{H_TOTAL}" rx="12" fill="#0d1117"/>
  <rect width="{W}" height="{H_TOTAL}" rx="12" fill="url(#shimmer)"/>
  ...
  <!-- ── HEADER ── -->
  {avatar_el}
  <text x="80" y="36" fill="#e6edf3" font-size="16" font-weight="600">{_esc(p["name"])}</text>
  <text x="80" y="52" fill="#8b949e" font-size="12">@{_esc(p["login"])}</text>
  <text x="80" y="70" fill="#8b949e" font-size="11">{_esc(p["bio"][:60])}</text>
  <text x="{W - 20}" y="36" fill="#8b949e" font-size="11" text-anchor="end">
    {s["followers"]} followers · {s["following"]} following
  </text>

  <!-- divider -->
  <line x1="20" y1="{H_HEADER}" x2="{W - 20}" y2="{H_HEADER}" stroke="#21262d" stroke-width="1"/>
  ...
  <!-- divider -->
  <line x1="20" y1="{H_HEADER + H_STATS}" x2="{W - 20}" y2="{H_HEADER + H_STATS}" stroke="#21262d" stroke-width="1"/>

  <!-- ── HEATMAP ── -->
  <text x="20" y="{H_HEADER + H_STATS + 14}" fill="#8b949e" font-size="10">
    {s["year_count"]} gist{'s' if s["year_count"] != 1 else ''} in the last year
  </text>
  {''.join(heat_rects)}

  <!-- divider -->
  <line x1="20" y1="{H_HEADER + H_STATS + H_HEAT}" x2="{W - 20}" y2="{H_HEADER + H_STATS + H_HEAT}" stroke="#21262d" stroke-width="1"/>

  <!-- ── LANGUAGES ── -->
  <text x="20" y="{lang_y0 - 6}" fill="#8b949e" font-size="10">LANGUAGES</text>
  {''.join(lang_rects)}
  {''.join(lang_labels)}

  <!-- divider -->
  <line x1="20" y1="{H_HEADER + H_STATS + H_HEAT + H_LANG}" x2="{W - 20}" y2="{H_HEADER + H_STATS + H_HEAT + H_LANG}" stroke="#21262d" stroke-width="1"/>

  <!-- ── RECENT GISTS ── -->
  <text x="20" y="{recent_y0 - 8}" fill="#8b949e" font-size="10">RECENT GISTS</text>
  {''.join(recent_rows)}
...
def _stat_block(x, y, value, label):
    return (
        f'<text x="{x}" y="{y + 14}" fill="#e6edf3" font-size="18" font-weight="700">{value}</text>'
        f'<text x="{x}" y="{y + 28}" fill="#8b949e" font-size="10">{label}</text>'
    )
